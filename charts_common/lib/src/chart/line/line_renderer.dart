...
class LineRenderer<D> extends BaseCartesianRenderer<D> {
  ...
  factory LineRenderer({String? rendererId, LineRendererConfig<D>? config}) {
    return LineRenderer._internal(rendererId: rendererId ?? 'line', config: config ?? LineRendererConfig<D>());
  }
  ...
  List<_DatumPoint<D>> _createBoundsSegment(
    ...
    areaPointList.addAll(pointList.map((datumPoint) => _DatumPoint.from(
        ...
        initializeFromZero
            ...
            : measureAxis.getLocation(
                (series.measureUpperBoundFn!(datumPoint.index) ?? 0) + series.measureOffsetFn!(datumPoint.index)!))));
    ...
    areaPointList.addAll(pointList.reversed.map((datumPoint) => _DatumPoint.from(
        ...
        initializeFromZero
            ? datumPoint.y
            : measureAxis.getLocation(
                (series.measureLowerBoundFn!(datumPoint.index) ?? 0) + series.measureOffsetFn!(datumPoint.index)!))));
  ...
  bool _isPointBelowSeries(
    ...
    if (chartPoint.x < nearestPoint.x!) {
      ...
      rightPoint = nearestPointIdx < allPointsForSeries.length - 1 ? allPointsForSeries[nearestPointIdx + 1] : null;
    }
